...
public class NumPang extends Activity {
	...
	class GameView extends View {
		...
		int mTextSize;						// 숫자 텍스트의 크기
		int mEdgeThick;						// 경계선의 두께
		...
		Paint mTileEdgePaint, mTilePaint, mTextPaint;
		Paint mTimeEdgePaint, mTimeGaugePaint, mScorePaint;
		// 타일의 배경 색과 글자색
		int[] arBackColor = {0, Color.RED, Color.GREEN, Color.BLUE, Color.YELLOW, 
				Color.CYAN, Color.MAGENTA, Color.BLACK, Color.GRAY, Color.WHITE };
		int[] arTextColor = {0, Color.WHITE, Color.BLACK, Color.WHITE, Color.BLACK, 
		...
		public GameView(Context context) {
			...
			mTileEdgePaint.setColor(Color.BLACK);
			...
			for (int x=0;x<mWidth;x++) {
				for (int y=0;y<mHeight;y++) {
					...
					if (mStatus == REMOVE && mDeadTile[x][y]) {
						...
						texty = (mTextSize * mScale) / 2;
						mTileEdgePaint.setStrokeWidth(mEdgeThick * mScale);
					}

					// 타일 및 숫자 출력
					mTilePaint.setColor(arBackColor[mBoard[x][y]]);
					canvas.drawRect(rt, mTilePaint);
					canvas.drawRect(rt, mTileEdgePaint);
					mTextPaint.setColor(arTextColor[mBoard[x][y]]);
					canvas.drawText("" + mBoard[x][y], tx + mTileWidth/2, 
							ty + mTileHeight/2 + texty, mTextPaint);
				}
			}
		}
